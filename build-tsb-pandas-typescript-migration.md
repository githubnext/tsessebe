...
Completed through iter 265 (on main through iter ~258, pending-ci 259–265):
...
- ✅ case_when — caseWhen(series, caselist) SQL-style CASE WHEN (iter 265)

Next:
- `hashPandasObject` / `Styler` / `where-aligned` — already pushed, pending-ci approval
- `core/str_accessor` — wire `.str.extractall()` via late-binding
- `pd.array` / extension array types
