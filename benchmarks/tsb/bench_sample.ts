...
for (let i = 0; i < 3; i++) s.sample(1000);
const N = 100;
const t0 = performance.now();
for (let i = 0; i < N; i++) s.sample(1000);
const elapsed = performance.now() - t0;
console.log(JSON.stringify({ function: "sample", mean_ms: elapsed / N, iterations: N, total_ms: elapsed }));
