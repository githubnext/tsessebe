...
for (let i = 0; i < 3; i++) s.nsmallest(10);
const N = 100;
const t0 = performance.now();
for (let i = 0; i < N; i++) s.nsmallest(10);
const elapsed = performance.now() - t0;
console.log(JSON.stringify({ function: "nsmallest", mean_ms: elapsed / N, iterations: N, total_ms: elapsed }));
