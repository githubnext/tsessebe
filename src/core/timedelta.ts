...
const PANDAS_RE = /^(-)?(\d+)\s+days?\s*(?:\+\s*)?(\d{1,2}):(\d{2}):(\d{2})(?:\.(\d+))?$/i;

/** Regex for "N unit" strings like "5 hours" or "1.5 days". */
const UNIT_RE =
  /^(-?\d+(?:\.\d+)?)\s*(ms|min|milliseconds?|seconds?|minutes?|hours?|days?|weeks?|[smhDdWw])$/i;
...
function parseTimedeltaString(raw: string): number {
  ...
  if (pm !== null) {
    ...
    const days = Number.parseInt(pm[2] ?? "0", 10);
    const hours = Number.parseInt(pm[3] ?? "0", 10);
    const minutes = Number.parseInt(pm[4] ?? "0", 10);
    const seconds = Number.parseInt(pm[5] ?? "0", 10);
    const fracStr = pm[6] ?? "";
    const fracMs =
      fracStr.length > 0
        ? Math.round(Number.parseInt(fracStr, 10) / 10 ** (fracStr.length - 3))
        : 0;
    const total =
      days * 86_400_000 + hours * 3_600_000 + minutes * 60_000 + seconds * 1_000 + fracMs;
  ...
  if (um !== null) {
    const value = Number.parseFloat(um[1] ?? "0");
