...
export class DataFrameGroupBy {
  ...
  /** Shorthand for `agg("sum")` — operates on numeric columns only. */
  sum(): DataFrame {
    const valueCols = this._valueCols(true);
    const colSpecs = this._resolveColSpecs("sum", valueCols);
    return this._runAgg(colSpecs, true);
  }
  /** Shorthand for `agg("mean")` — operates on numeric columns only. */
  mean(): DataFrame {
    const valueCols = this._valueCols(true);
    const colSpecs = this._resolveColSpecs("mean", valueCols);
    return this._runAgg(colSpecs, true);
  }
  ...
  /** Shorthand for `agg("std")` — operates on numeric columns only. */
  std(): DataFrame {
    const valueCols = this._valueCols(true);
    const colSpecs = this._resolveColSpecs("std", valueCols);
    return this._runAgg(colSpecs, true);
  }
  ...
  private _valueCols(numericOnly = false): readonly string[] {
    const bySet = new Set(this._by);
    return this._df.columns.toArray().filter((c) => {
      if (bySet.has(c)) {
        return false;
      }
      if (numericOnly) {
        return this._df.col(c).dtype.isNumeric;
      }
      return true;
    });
  }
